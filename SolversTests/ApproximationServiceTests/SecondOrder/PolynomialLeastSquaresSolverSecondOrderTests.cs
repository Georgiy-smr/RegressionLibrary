...
public class PolynomialLeastSquaresSolverSecondOrderTests
{
    ...
    /// <summary>
    /// Same 224 dataset as ApproximationServiceTests224 (data generated for the cubic/16-term
    /// model), fit here with the 9-term/second-order basis (a0..a8, SecondOrderBasisExponents)
    /// instead. GetMax() actually observed: 0.013598238882707392. A 9-term basis fit to data
    /// generated for a cubic model is expected to be looser than the 16-term fit (0.011
    /// tolerance in ApproximationServiceTests224) — 0.02 below is an empirically chosen
    /// tolerance with headroom over that observed value, not a hard accuracy requirement.
    /// </summary>
    [Fact]
    public void MaxErrorIsWithinEmpiricalTolerance()
    {
        var sut = new PolynomialLeastSquaresSolver(new SecondOrderBasisExponents());
